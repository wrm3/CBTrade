# FSTRent Tasks Workflow Diagram

Below is the workflow diagram for the FSTrent_Tasks system, illustrating the process of task creation, execution, and archival.
...
graph TD
    ...
    B1 -- No --> C;
    B2 --> C["Update .fstrent_tasks_v2/TASKS.md with ALL Planned Tasks (Parent Tasks & Sub-tasks)"];
    C --> D["For Each Task (Parent Task or Sub-task) in .fstrent_tasks_v2/TASKS.md"];
    D -- Loop --> E[Create/Update Individual task file in .fstrent_tasks_v2/tasks/];
    ...
    S{User asks to archive?} --> T[Agent finds completed/failed tasks in .fstrent_tasks_v2/tasks/];
    T --> U[Move task files to .fstrent_tasks_v2/memory/tasks/];
    U --> V[Append summary to .fstrent_tasks_v2/memory/TASKS_LOG.md];
    V --> W[Remove corresponding entries from .fstrent_tasks_v2/TASKS.md];
```
