...
def buy_strat_settings_nwe_env(st):
...
def sell_strat_settings_nwe_env(st):
...
def buy_strat_nwe_env(buy, ta, pst):
	func_name = 'buy_strat_nwe_env'
	...
	try:
		...
		buy.buy_yn       = 'Y'
		buy.wait_yn      = 'N'
		buy.show_tests_yn = buy.pst.buy.strats.nwe_env.show_tests_yn
		...
		if pst.buy.strats.nwe_env.show_tests_yn == 'Y':
			show_stuff(freq, ta, False)

		nwe_env_buy_signal          = buy.ta[freq]['nwe_env_buy_signal']['ago0']
		nwe_env_sell_signal         = buy.ta[freq]['nwe_env_sell_signal']['ago0']
		nwe_env_buy_signal_last     = buy.ta[freq]['nwe_env_buy_signal']['ago1']
		...
		if nwe_env_buy_signal:
			msg = f'    * BUY SIGNAL: NWE_ENV {freq} Envelope Breach detected (BULL)'
...
def sell_strat_nwe_env(mkt, pos, ta, pst):
	func_name = 'sell_strat_nwe_env'
	...
	try:
		...
		if pst.sell.strats.nwe_env.show_tests_yn == 'Y':
			show_stuff(freq, ta, False)
		...
		nwe_env_buy_signal             = ta[freq]['nwe_env_buy_signal']['ago0']
		nwe_env_sell_signal            = ta[freq]['nwe_env_sell_signal']['ago0']
		nwe_env_sell_signal_last       = ta[freq]['nwe_env_sell_signal']['ago1']

		if pos.prod_id == 'BTC-USDC':
			if nwe_env_sell_signal:
				speak_async(f"{pos.prod_id} has a Nadaraya Watson Envelope Sell Trigger on the {freq} timeframe")

		# if nwe_rev_sell_signal or (nwe_rev_sell_signal_last and not nwe_rev_buy_signal):
		# 	pos.sell_yn = 'Y'
		# 	pos.hodl_yn = 'N'
		# 	msg = f'    * SELL SIGNAL: NWE_ENV {freq} Trend Reversal (BEAR)'
		# 	pos.reason = msg.strip()
		# 	all_sells.append(msg)
		# el
#		if nwe_env_sell_signal or (nwe_env_sell_signal_last and not nwe_env_buy_signal):
		if nwe_env_sell_signal:
			pos.sell_yn = 'Y'
			pos.hodl_yn = 'N'
			msg = f'    * SELL SIGNAL: NWE_ENV {freq} Trend Envelope Reversal (BEAR)'
			pos.reason = msg.strip()
			all_sells.append(msg)
		...
		exit_if_profit_yn      = pst.sell.strats.nwe_env.exit_if_profit_yn
		exit_if_profit_pct_min = pst.sell.strats.nwe_env.exit_if_profit_pct_min
		exit_if_loss_yn        = pst.sell.strats.nwe_env.exit_if_loss_yn
		exit_if_loss_pct_max   = abs(pst.sell.strats.nwe_env.exit_if_loss_pct_max) * -1
		if pos.sell_yn == 'Y':
