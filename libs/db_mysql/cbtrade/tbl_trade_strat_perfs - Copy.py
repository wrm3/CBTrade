...
def db_trade_strat_perfs_exists(self):
    ...
    sql = '''
        CREATE TABLE IF NOT EXISTS `trade_strat_perfs` (
            ...
            `strat_bo_elapsed` DECIMAL(36,12) DEFAULT 9999.0,
            `strat_pos_elapsed` DECIMAL(36,12) DEFAULT 9999.0,
            `strat_last_elapsed` DECIMAL(36,12) DEFAULT 9999.0,
...
@narc(1)
def db_trade_strat_perfs_get(self, base_symb:str=None, quote_symb:str=None, prod_id:str=None, buy_strat_type:str=None, buy_strat_name:str=None, buy_strat_freq:str=None, lta:str=None, min_trades:int=0, force_recalc:bool=False):
      ...
      , UNIX_TIMESTAMP() - GREATEST(
                                        ...
                                        COALESCE(
                                            (SELECT MAX(b.buy_begin_unix) 
                                             ...
                                             AND b.buy_strat_freq = %s 
                                             AND (CASE 
                                                  WHEN %s = 'L' THEN b.test_txn_yn = 'N'
                                                  WHEN %s = 'T' THEN b.test_txn_yn = 'Y'
                                                  ELSE 1=1 
                                                  END)
                                            ), 0
                                        )
                                    ) AS strat_last_elapsed
...
@narc(1)
def db_trade_strat_perfs_recalc_single_old(self, prod_id, buy_strat_type, buy_strat_name, buy_strat_freq, lta):
    if self.debug_tf: G(f'==> CBTRADE_DB.db_trade_strat_perfs_recalc_single(prod_id={prod_id}, buy_strat_type={buy_strat_type}, buy_strat_name={buy_strat_name}, buy_strat_freq={buy_strat_freq}, lta={lta})')
    """
    Internal method to calculate performance for a single LTA type
    """
