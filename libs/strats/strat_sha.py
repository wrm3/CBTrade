...
def ta_add_sha(df: pd.DataFrame, prc_mkt, sha_len1=5, sha_len2=8, tag=None) -> pd.DataFrame:
    ...
    """
    # 🚨 CRITICAL FIX: Prevent index out of bounds - DataFrame indices are 0-based
    ...
    if sha_len1 < 2 or sha_len2 < 2 or len(df) < 3:
        print(f"\n{'='*80}")
        print(f"🚨 INSUFFICIENT DATA FOR SHA STRATEGY CALCULATION")
        print(f"{'='*80}")
...
def buy_strat_sha(buy, ta, st_pair, curr_prc=None):
    ...
    - Generates a buy signal if:
          • The current price is above both the fast and slow SHA close.
          • The fast SHA candle body is growing and top-heavy.
          • The slow SHA candle body is growing and top-heavy.
          • Overall candle color and Heikin-Ashi color are green.
...
def sell_strat_sha(mkt, pos, ta, st_pair, curr_prc=None):
    ...
    try:
        ...
        - Generates a sell signal if:
            • The current price is below both the fast and slow SHA close.
            • The fast SHA candle body is shrinking and bottom-heavy.
            • The slow SHA candle body is shrinking and bottom-heavy.
            • Overall candle color and Heikin-Ashi color are red.
        ...
        print(f"\n{'='*80}")
        print(f"🚨 CRITICAL SELL_STRAT_SHA PROCESSING FAILURE")
        print(f"{'='*80}")
