...
def calculate_dynamic_lengths(df, base_length, volatility_window=20):
    ...
    Returns base_length for all rows if insufficient data.
    """
    # Check if we have enough data for ATR calculation
    if len(df) < volatility_window:
        # Insufficient data - return base_length for all rows
        return pd.Series([base_length] * len(df), index=df.index)
    
    # Calculate ATR
    atr = pta.atr(df['high'], df['low'], df['close'], length=volatility_window)
    
    # Handle case where ATR calculation fails (returns None)
    if atr is None:
        return pd.Series([base_length] * len(df), index=df.index)
    
    atr_ma = atr.rolling(window=volatility_window).mean()
    
    # Calculate volatility ratio with NaN/inf protection
    volatility_ratio = atr / atr_ma
    
    # Replace invalid values with 1.0 (neutral volatility)
    volatility_ratio = volatility_ratio.replace([np.inf, -np.inf, np.nan], 1.0)
    
    # Ensure volatility_ratio is never zero to prevent division errors
    volatility_ratio = np.maximum(volatility_ratio, 0.1)
    ...
    # Handle any remaining NaN/inf values before conversion
    dynamic_length = dynamic_length.fillna(base_length)
    dynamic_length = dynamic_length.replace([np.inf, -np.inf], base_length)
...
def ta_add_sha(df: pd.DataFrame, prc_mkt, sha_len1=5, sha_len2=8, tag=None) -> pd.DataFrame:
    ...
    ✅ ARCHITECTURE: Data is pre-validated by consolidated OHLCV pipeline
    """
    # Simple validation - data should already be protected upstream
    if df.empty:
        print(f"⚠️ WARNING: ta_add_sha{tag or ''}: Empty DataFrame received")
        for col in ['sha_open', 'sha_close', 'sha_high', 'sha_low', 'sha_color']:
            df[col] = np.nan
        return df
    ...
    if tag:
        ...
        columns_to_copy = [
            ...
            'sha_body', 'sha_body_growing', 'sha_body_shrinking',
            'sha_wick_upper', 'sha_wick_upper_growing', 'sha_wick_upper_shrinking',
            'sha_wick_lower', 'sha_wick_lower_growing', 'sha_wick_lower_shrinking',
            'sha_wick_top_heavy', 'sha_wick_bottom_heavy',
...
def buy_strat_sha(buy, ta, st_pair, curr_prc=None):
    ...
    try:
        ...
        rsi_threshold = st_pair.strats.sha.rsi_threshold
        bull_strength_threshold = st_pair.strats.sha.bull_strength_threshold
...
def sell_strat_sha(mkt, pos, ta, st_pair, curr_prc=None):
    ...
    try:
        ...
        slow_sha_len1 = st_pair.strats.sha.slow_sha_len1
        slow_sha_len2 = st_pair.strats.sha.slow_sha_len2
        bear_strength_threshold = st_pair.strats.sha.bear_strength_threshold
        ...
        error_msg = f"""
=== CRITICAL SHA SELL STRATEGY EXCEPTION ===
File: {__file__}
Function: {sys._getframe().f_code.co_name}
