        ...
        if self.mkt.refresh_wallet_tf or force_tf:
            ...
            for bal in bals:
                ...
                if curr == self.mkt.symb:
                    ...
                    # Apply select pair reserve logic
                    total_reserve = self.budget[self.mkt.symb].reserve_amt
                    select_pair_reserve = self.mst.budget.select_pair_reserve_amt
                    select_pair_pairs = self.mst.budget.select_pair_pairs
                    
                    # If this is NOT a select pair, add the select pair reserve to total reserve
                    # During initialization, pair.prod_id may not exist yet - default to applying reserve
                    if not hasattr(self, 'pair') or not hasattr(self.pair, 'prod_id') or self.pair.prod_id not in select_pair_pairs:
                        total_reserve += select_pair_reserve
                    
                    self.budget[self.mkt.symb].spendable_amt = self.budget[self.mkt.symb].bal_avail - total_reserve
