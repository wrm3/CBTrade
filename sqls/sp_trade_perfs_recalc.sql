...
-- Does INSERT ... ON DUPLICATE KEY UPDATE to trade_perfs, then SELECTs the row back
use cbtrade;

DROP PROCEDURE IF EXISTS sp_trade_perfs_recalc;

DELIMITER $$

CREATE PROCEDURE sp_trade_perfs_recalc (
    IN p_prod_id         VARCHAR(64),   -- pass NULL to recalc ALL prod_ids
    IN p_lta             CHAR(1)        -- 'L' (live), 'T' (test), 'A' (all). Pass NULL to write for all L/T/A
)
BEGIN
    /* Multi-target upsert: when p_prod_id or p_lta are NULL we fan out using GROUP BY */
    INSERT INTO trade_perfs (
        base_symb
        , quote_symb
        , prod_id
        , lta
    ...
    SELECT
      ...
      /* Elapsed metrics from buy_ords & poss (respecting L/T based on lt.lta) */
             ...
             THEN FROM_UNIXTIME((SELECT MAX(b.buy_begin_unix)
                                ...
                                WHERE b.prod_id = pl.prod_id
                                  AND (CASE WHEN lt.lta='L' THEN b.test_txn_yn='N'
                                            WHEN lt.lta='T' THEN b.test_txn_yn='Y'
                                            ELSE 1=1 END)))
             ELSE NULL END AS last_buy_bo_dttm
             ...
             THEN FROM_UNIXTIME((SELECT MAX(p2.pos_begin_unix)
                                ...
                                WHERE p2.prod_id = pl.prod_id
                                  AND (CASE WHEN lt.lta='L' THEN p2.test_txn_yn='N'
                                            WHEN lt.lta='T' THEN p2.test_txn_yn='Y'
                                            ELSE 1=1 END)))
             ELSE NULL END AS last_buy_pos_dttm
      ...
      , CASE WHEN GREATEST(
            ...
            COALESCE( (SELECT MAX(b2.buy_begin_unix)
             ...
             ELSE NULL END AS last_buy_dttm
      , GREATEST(
            COALESCE( (SELECT MAX(p3.pos_begin_unix)
                       FROM poss p3
                       WHERE p3.prod_id = pl.prod_id
    ...
    FROM 
      ...
      (SELECT p_prod_id AS prod_id
       WHERE p_prod_id IS NOT NULL
       UNION
      ...
      /* L/T/A list based on param (NULL => all three) */
      JOIN (
      ...
      /* aggregate over poss with left join so we still upsert even if no rows */
      LEFT JOIN poss p
    ...
    ON DUPLICATE KEY UPDATE
        ...
        last_buy_bo_dttm = VALUES(last_buy_bo_dttm),
        last_buy_bo_unix = VALUES(last_buy_bo_unix),
        last_buy_pos_dttm = VALUES(last_buy_pos_dttm),
        last_buy_pos_unix = VALUES(last_buy_pos_unix),
        last_buy_dttm = VALUES(last_buy_dttm),
        last_buy_unix = VALUES(last_buy_unix),
